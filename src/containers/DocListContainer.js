...
export class DocListContainer extends React.Component {
  ...
  render() {
    ...
    if (this.props.docs !== undefined) {
      const filenames = Object.keys(this.props.docs);

      // Filter files based on search string
      const filteredFilenames = filenames.filter((filename) =>
        filename.toLowerCase().includes(this.state.searchValue.toLowerCase()),
      );
    ...
    return (
      <DocList docList={fileList} onSearchChange={this.handleSearchChange} />
    );
  }
}
